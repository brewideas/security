...
import com.infius.proximitysecurity.listeners.CameraListener;
...
public class CameraSource {
    ...
    float mDist = 0;
    private MotionEvent distSpacing;
    ...
    public void setDistSpacing(MotionEvent event) {
    ...
    private Thread mInvertProcessingThread;
    private InvertedFrameProcessingRunnable mInvertedFrameProcessor;
    ...
    public static class Builder {
        private final List<Detector<?>> mDetectorList;
         ...
         */
        public Builder(Context context, List<Detector<?>> detectorList) {
            if (context == null) {
        ...
        public CameraSource build() {
            mCameraSource.mFrameProcessor = mCameraSource.new FrameProcessingRunnable(mDetectorList);
//            mCameraSource.mInvertedFrameProcessor = mCameraSource.new InvertedFrameProcessingRunnable(mDetectorList);
...
//            mInvertedFrameProcessor.release();
        }
    }
    ...
    public CameraSource start() throws IOException {
        synchronized (mCameraLock) {
            ...
            try {
                ...
                mCamera = null;
                Toast.makeText(mContext, "Can't create Camera", Toast.LENGTH_SHORT).show();
                if (cameraListener != null) {
                    cameraListener.onCameraCreateFailed();
                }
                e.printStackTrace();
            }
...
//                mInvertProcessingThread = new Thread(mInvertedFrameProcessor);
//                mInvertedFrameProcessor.setActive(true);
//                mInvertProcessingThread.start();
            }
        }
        return this;
    }
...
//            mInvertedFrameProcessor.setActive(false);
...
//            if (mInvertProcessingThread != null) {
//                try {
//                    // Wait for the thread to complete to ensure that we can't have multiple threads
//                    // executing at the same time (i.e., which would happen if we called start too
//                    // quickly after stop).
//                    mInvertProcessingThread.join();
//                } catch (InterruptedException e) {
//                    Log.d(TAG, "Frame processing thread interrupted on release.");
//                }
//                mInvertProcessingThread = null;
//            }
...
//            Log.e("Kshemendra", "size.getWidth() : " + size.getWidth() + " size.getHeight() " + size.getHeight());
...
//        Log.e("Kshemendra", "selectedPair.getWidth() : " + selectedPair.previewSize().getWidth() + " selectedPair.getHeight() " + selectedPair.previewSize().getHeight());
        return selectedPair;
    }
...
//            mInvertedFrameProcessor.setNextFrame(data, camera);
        }
    }
    ...
    private class FrameProcessingRunnable implements Runnable {
        ...
        FrameProcessingRunnable(List<Detector<?>> detector) {
            mDetectorList = detector;
        }
        ...
        void release() {
            ...
            for (Detector<?> detector : mDetectorList) {
                detector.release();
                detector = null;
            }
        }
         ...
         * continuously.  The next pending frame is either immediately available or hasn't beenparameters.setColorEffect
    ...
    private class InvertedFrameProcessingRunnable implements Runnable {
        ...
        InvertedFrameProcessingRunnable(List<Detector<?>> detector) {
            mDetectorList = detector;
        }
        ...
        @SuppressLint("Assert")
        void release() {
            assert (mInvertProcessingThread.getState() == State.TERMINATED);
            for (Detector<?> detector : mDetectorList) {
                detector.release();
                detector = null;
            }
        ...
        public void run() {
            ...
            while (true) {
                synchronized (mLock) {
                    ...
                    outputFrame = getInvertedFrame(mPendingFrameData, outputFrame);
    ...
    private Frame getInvertedFrame(ByteBuffer pendingFrameBuffer, Frame pendingFrame) {
        byte[] bytes = pendingFrameBuffer.array();

        int width = pendingFrame.getMetadata().getWidth();
        int height = pendingFrame.getMetadata().getHeight();

        int[] pixels = applyGrayScale(bytes, width, height);
        ...
        encodeYUV420SP(bytes, pixels, bm.getWidth(), bm.getHeight());

        ByteBuffer invertedFrameBuffer = ByteBuffer.wrap(bytes);

        return new Frame.Builder()
                .setImageData(invertedFrameBuffer, width, height, ImageFormat.NV21)
                .setId(pendingFrame.getMetadata().getId())
                .setTimestampMillis(pendingFrame.getMetadata().getTimestampMillis())
                .setRotation(pendingFrame.getMetadata().getRotation())
                .build();
    }

    private byte[] encodeYUV420SP(byte[] yuv420sp, int[] argb, int width, int height) {
    ...
    private static Bitmap createInvertedBitmap(Bitmap src, Integer width, Integer height) {
